--[[
	...
	Revision: $Id: PostMonitor.lua 6767 2022-01-25 11:42:44Z none $
...
LibStub("LibRevision"):Set("$URL: BeanCounter/PostMonitor.lua $","$Rev: 6767 $","5.1.DEV.", 'auctioneer', 'libs')
	...
	if (name and count) then
		...
		for bagID = 0, 4 do
			...
			for slot = 1, bagSlots do
				...
				if info then
					local selectedStack = info.stackCount or 0
					...
					if locked then
						...
						if link and strfind(link, name, 1, true) then
							itemLink = link
							selectedStackCount = selectedStack
							break
						end
					end
				end
		...
		local deposit = GetAuctionDeposit (runTime, minBid, buyoutPrice )
...
function private.bagSnapshot(itemBeingPosted, totalQuantity)
    private.multipostScan1 = {}
    local counter = 0
        ...
        if slots then
            for slot = 1, slots do
                ...
                if info and link then
                    local itemCount = info.stackCount or 0
                    local itemID = lib.API.decodeLink(link)

                    if itemID == itemBeingPosted then
                        table.insert(private.multipostScan1, 1, {
                            count = itemCount,
                            link = link
                        })
