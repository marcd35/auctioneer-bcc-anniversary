--[[
	...
	Version: 2.5.6772 (SwimmingSeadragon)
	Revision: $Id: Stubby.lua 6772 2022-01-25 11:42:44Z none $
...
LibStub("LibRevision"):Set("$URL: Stubby/Stubby.lua $","$Rev: 6772 $","5.1.DEV.", 'auctioneer', 'libs')
...
function registerAddOnHook(triggerAddOn, ownerAddOn, hookFunction, ...)
	if (C_AddOns.IsAddOnLoaded(triggerAddOn)) then
		if (select("#", ...) == 0) then
			hookFunction()
		else
			hookFunction({...})
...
function checkAddOns()
	...
	for i=1, addonCount do
		name, title, notes = C_AddOns.GetAddOnInfo(i)
...
function cleanUpAddOnData()
	...
	for b in pairs(StubbyConfig.boots) do
		local _,title = C_AddOns.GetAddOnInfo(b)
		if (not title) then
	...
	for i=1, C_AddOns.GetNumAddOns() do
		if C_AddOns.IsAddOnLoadOnDemand(i) then
			...
			if shouldInspectAddOn(name) and (reason == "DEMAND_LOADED" or reason == "DEP_DEMAND_LOADED") then
				for _, dependancy in pairs({C_AddOns.GetAddOnDependencies(i)}) do
...
function runBootCodes()
	...
	for addon, boots in pairs(StubbyConfig.boots) do
		if (not C_AddOns.IsAddOnLoaded(addon) and C_AddOns.IsAddOnLoadOnDemand(addon)) then
			local _, _, _, _, reason = C_AddOns.GetAddOnInfo(addon)
...
function getRevision()
	return tonumber(("$Rev: 6772 $"):match("(%d+)"))
end
