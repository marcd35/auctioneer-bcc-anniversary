--[[
	...
	Revision: $Id: CoreSettings.lua 6733 2022-01-25 11:42:44Z none $
	...
	if (a == "profile") then
		if setting == "profile.save" or setting == "profile.duplicate" then
			...
			if value and value ~= "" then
				...
				print(_TRANS("ADV_Help_InvalidProfileName")) --"Cannot create new profile: please enter a new profile name first"
				return
			end
		elseif (setting == "profile.delete") then
			...
			if value and value ~= "Default" then -- don't let the user delete the Default profile!
				...
				print(_TRANS("ADV_Help_CannotDeleteProfile")) --"The selected profile cannot be deleted"
				return
			end
	...
	elseif a == "maintenance" then
		if setting == "maintenance.clearscan" then
			if IsAltKeyDown() and not IsShiftKeyDown() and not IsControlKeyDown() then -- Alt key and NO other mod keys
				...
				print("ScanData cleared")
			else
				print("You must hold down the Alt key to clear all ScanData")
			end
		end
...
AucAdvanced.RegisterRevision("$URL: Auc-Advanced/CoreSettings.lua $", "$Rev: 6733 $")
