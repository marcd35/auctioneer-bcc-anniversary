--[[
	ScrollSheet
	Version: 2.5.6736 (SwimmingSeadragon)
	Revision: $Id: ScrollSheet.lua 6736 2022-01-25 11:42:44Z none $
...
LibStub("LibRevision"):Set("$URL: Auc-Advanced/Libs/Configator/ScrollSheet.lua $","$Rev: 6736 $","5.1.DEV.", 'auctioneer', 'libs')
...
function kit:Render()
	...
	for i = 1, #rows do
		...
		rows[i].colorTex:SetGradient(direction,CreateColor(rowR, rowG, rowB, rowA1),CreateColor(rowR, rowG, rowB, rowA2))
	end
	self:RowSelect()
end
...
function lib:Create(frame, layout, onEnter, onLeave, onClick, onResize, onSelect)
	...
	for i = 1, #layout do
		...
		button:SetResizeBounds(0, 0, 400, 16)
...
function  lib.moveColumn(self, column)
	if self and column then
		if IsControlKeyDown() then --reset column to default
			...
			self.labels[column].button:SetScript("OnUpdate", function() lib.changeColumns(self, column, GetMouseFoci()[1]) end)
		end
	end
end
function lib.changeColumns(self, column, button)
	...
	if not IsMouseButtonDown() and self.moving and GetMouseFoci()[1]:GetID() > 0 then
		local movingTo = GetMouseFoci()[1]:GetID()
