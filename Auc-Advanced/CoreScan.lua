		...
		Committing a scan.
			...
			Stage 3.  Merge new scan with AH Image
				...
				if a match is found in AH Image that still needs resolved
					if auction in AH Image was not filtered then
						...
						if not, then send 'update' processor print.
						otherwise send 'leave' processor print.
					update item in AH Image from scan info and mark entry as resolved.
				otherwise
					send 'filter' processor print.
					If no filters indicate auction to be filtered, then
						send 'create' processor print.
					otherwise
			...
			Stage 4.  Remove unseen from AH Image
				...
				if needs resolved then
					if expired, then remove from image, sending 'delete' processor print
					otherwise if not expired and a complete scan, remove from image, sending 'delete' process print
					otherwise if flagged unseen prior scan, remove from image sending 'delete' processor print
...
function private.LoadScanData()
	if not private.loadingScanData then
		local _, _, _, _, reason = C_AddOns.GetAddOnInfo("Auc-ScanData")
		if C_AddOns.IsAddOnLoaded("Auc-ScanData") then
		...
		elseif reason ~= "DEMAND_LOADED" then -- unable to be loaded
			...
			local loaded, reason = C_AddOns.LoadAddOn("Auc-ScanData")
			if loaded then
				private.loadingScanData = "loading"
			elseif reason then
				private.loadingScanData = "fallback"
...
function lib.StartScan(name, minUseLevel, maxUseLevel, isUsable, qualityIndex, GetAll, exactMatch, filterData, options) -- ### Legion : revised, check
	if AuctionFrame and AuctionFrame:IsVisible() then
		if private.isPaused then
			print("Scanning is currently paused")
			return
		end
		if private.isScanning then
			print("Scan is currently in progress")
			return
		end
		...
		if not private.curQuery then
			...
			print("Scan failed: unable to send query")
			if private.isGetAll then
		...
		print("Steady on; You'll need to talk to the auctioneer first!")
	end
end
...
local Commitfunction = function()
	...
	-- Send ScanProcessor print "begin"
	...
	do --[[ *** Stage 2 : Pre-process image table ***
		...
		Do not copy expired (or corrupt) entries; issue "delete" processor print for these
	...
	do --[[ *** Stage 3 : Merge new scan into ScanData *** ]]
		...
		for index, data in ipairs(TempcurScan) do
			...
			if (itemPos) then
				...
				local oldSellerWasUnknown = (oldItem[Const.SELLER] == "" or oldItem[Const.SELLER] == nil)
				...
				if bitand(data[Const.FLAG], Const.FLAG_FILTER) ~= 0 then
					...
					if oldSellerWasUnknown and data[Const.SELLER] and data[Const.SELLER] ~= "" then
						if not processStats(processors, messageCreate, data) then
							-- Filter matched: mark this existing auction as filtered
...
function private.StopStorePage(silent)
	...
	if isGetAll and not silent then
		_G.print("Warning: GetAll scan is incomplete because it was interrupted")
	end
end
	...
	if itemData then
		...
		for pos, engineLib in ipairs(modules) do
			if engineLib.ScanProcessors["newauc"] then
				local pOK, errormsg = pcall(engineLib.ScanProcessors["newauc"],"newauc", statItem, minBid, buyoutPrice, runTime, price )
				if not pOK then
	...
	if isVisibleAucFrame then
		...
		if private.sentQuery then
			...
			elseif timeoutSentQuery > 600 then
				...
				timeoutSentQuery = 0
				private.ResetAll()
				print("Auctioneer: Scan failed, Server is not responding")
			else
...
AucAdvanced.RegisterRevision("$URL: Auc-Advanced/CoreScan.lua $", "$Rev: 6733 $")
