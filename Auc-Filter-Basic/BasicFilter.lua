...
function lib.AuctionFilter(operation, itemData)
	...
	-- Quality and item level filters: only apply on initial "create" to avoid
	-- re-filtering existing auctions whose quality/level data may differ slightly.
	if operation == "create" or operation == "fallbackcreate" then
	...
	-- Ignore list (seller name): apply at any operation so that auctions whose
	-- seller names were empty ("") on first scan due to deferred GUID resolution
	-- in patch 2.5.6 are still caught when the name resolves on subsequent scans.
...
local function SetupConfigGui(gui)
	...
	StaticPopupDialogs["BASICFILTER_ADD_IGNORE"] = {
		...
		OnAccept = function(self)
			lib.AddPlayerIgnore((self.editBox or self.EditBox):GetText())
		end,
		...
		OnHide = function(self)
			...
			(self.editBox or self.EditBox):SetText("");
		end,
		EditBoxOnEnterPressed = function(self)
			local parent = self:GetParent()
			lib.AddPlayerIgnore((parent.editBox or parent.EditBox):GetText())
			parent:Hide()
		end,
...
AucAdvanced.RegisterRevision("$URL: Auc-Filter-Basic/BasicFilter.lua $", "$Rev: 6759 $")
