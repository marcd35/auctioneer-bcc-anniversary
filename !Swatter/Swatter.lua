--[[
	...
	Version: 2.5.6773 (SwimmingSeadragon)
	Revision: $Id: Swatter.lua 6773 2022-01-25 11:42:44Z none $
...
for addon, name in pairs(otherdebug) do
	local enabled = C_AddOns.GetAddOnEnableState(player, addon)
...
LibStub("LibRevision"):Set("$URL: !Swatter/Swatter.lua $","$Rev: 6773 $","6.0.DEV.", 'auctioneer', 'libs')
...
function Swatter.GetAddOns()
	...
	for i = 1, C_AddOns.GetNumAddOns() do
		if C_AddOns.IsAddOnLoaded(i) then
			...
			local addition = C_AddOns.GetAddOnMetadata(i, "X-Swatter-Extra")
			if not version then
				version = C_AddOns.GetAddOnMetadata(i, "X-Version")
			end
			if not version then
				version = ""
			end
...
Swatter.Error.Done = CreateFrame("Button", "SwatterErrorDoneButton", Swatter.Error, "UIPanelButtonTemplate")
...
Swatter.Error.Next = CreateFrame("Button", "SwatterErrorNextButton", Swatter.Error, "UIPanelButtonTemplate")
...
Swatter.Error.Prev = CreateFrame("Button", "SwatterErrorPrevButton", Swatter.Error, "UIPanelButtonTemplate")
